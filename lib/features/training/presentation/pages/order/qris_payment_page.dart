...
class _QrisPaymentPageState extends State<QrisPaymentPage> {
  ...
  Future<void> _fetchMidtransQris() async {
    ...
    if (mounted) {
      setState(() {
        _qrisPayload =
            '00020101021126570011ID.CO.QRIS.WWW011893600915300000000002141530000000000303UMI51440014ID.CO.QRIS.WWW0215ID10190000000000303UMI5204599953033605406${widget.totalBayar}.005802ID5913Safenesia K36006Jakarta61051234562070703A016304C63E';
        _isLoading = false;
      });
      _startTimer();
    }
  ...
  Widget build(BuildContext context) {
    final String deadlineStr = DateFormat(
      'dd MMM yyyy, HH:mm',
    ).format(deadline);
                          ...
                          children: [
                            ...
                            IconButton(
                              ...
                              onPressed: () {
                                Clipboard.setData(
                                  ClipboardData(
                                    text: widget.totalBayar.toString(),
                                  ),
                                );
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                ...
                                onPressed: () async {
                                  try {
                                    ...
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      const SnackBar(
                                        content: Text(
                                          'Invoice berhasil disimpan ke Perangkat',
                                        ),
                                      ),
                                    );
                                  } catch (e) {
